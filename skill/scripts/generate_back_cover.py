#!/usr/bin/env python3
"""Generate paperback BACK cover artwork via OpenRouter Gemini 3 Pro Image.

Generates the ARTWORK only — book blurb / ISBN barcode are overlaid in a
vector editor. Counterpart to generate_front_cover.py with the same
config + prompt-file + multi-ref pattern.

Tip: write the back-cover prompt so the right third (or wherever your
blurb will sit) is held quiet — darker, less busy, no faces in that
zone. The model honors compositional hints reliably if you state them
in the COMPOSITION block of the prompt.

Cost: ~$0.14 per generation.

Usage:
    python3 generate_back_cover.py
    python3 generate_back_cover.py --prompt-file prompts/Covers/back-alt.txt
...
DEFAULT_PROMPT_REL = "prompts/Covers/back-prompt.txt"
...
def main():
    ...
    ap.add_argument("--basename", default="back-cover-art", help="Output filename stem")
    ap.add_argument("--refs", nargs="*", help="Override reference image paths (default: protagonist.strong_refs)")
    add_config_arg(ap)
    args = ap.parse_args()
